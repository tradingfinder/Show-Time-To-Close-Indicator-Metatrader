...
input color Clr = clrDodgerBlue;  // Color
...
//---
   ...
   if(Period()!=PERIOD_MN1 && Period()!=PERIOD_W1)
      TimeShow = TimeToString(PeriodSeconds(PERIOD_CURRENT) - (TimeCurrent() - time[rates_total-1]), TIME_SECONDS);
   else
     {
      int next = PeriodSeconds(PERIOD_CURRENT) - int(TimeCurrent() - time[rates_total-1]);
      TimeShow = IntegerToString((next/86400)-1)+"D & " + TimeToString(PeriodSeconds(PERIOD_CURRENT) - (TimeCurrent() - time[rates_total-1]), TIME_SECONDS);
     }
